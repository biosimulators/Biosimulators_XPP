...
We enthusiastically welcome contributions to BioSimulators-XPP!
...
* `biosimulators_xpp/`: Python code for a BioSimulators-compliant command-line interface to XPP
...
* `LICENSE`: License
* `CONTRIBUTING.md`: Guide to contributing to BioSimulators-XPP (this document)
* `CODE_OF_CONDUCT.md`: Code of conduct for developers of BioSimulators-XPP

## Coding convention

BioSimulators-XPP follows standard Python style conventions:
...
We strive to have complete test coverage for BioSimulators-XPP.

The unit tests for BioSimulators-XPP are located in the `tests`  directory. The tests can be executed by running the following command:
```
pip install pytest
...
The coverage of the tests can be evaluated by running the following commands and then opening `/path/to/biosimulators_xpp/htmlcov/index.html` with your browser.
```
pip install pytest pytest-cov coverage
python -m pytest tests --cov biosimulators_xpp
coverage html
```

## Documentation convention

BioSimulators-XPP is documented using [reStructuredText](https://www.sphinx-doc.org/en/master/usage/restructuredtext/index.html) and the [napoleon Sphinx plugin](https://www.sphinx-doc.org/en/master/usage/extensions/napoleon.html). The documentation can be compiled by running the following commands:
...
3. Increment the `__version__` variable in `biosimulators_xpp/_version.py`.
4. Commit this change to `biosimulators_xpp/_version.py`.
5. Add a tag for the new version by running `git tag { version }`. `version` should be equal to the value of the
   `__version__` variable in `biosimulators_xpp/_version.py`.
...
Please use [GitHub issues](https://github.com/biosimulators/Biosimulators_XPP/issues) to report any issues to the development community.

## Getting help

Please use [GitHub issues](https://github.com/biosimulators/Biosimulators_XPP/issues) to post questions or contact the lead developers at [info@biosimulators.org](mailto:info@biosimulators.org).
